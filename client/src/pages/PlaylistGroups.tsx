...
interface PlaylistGroupsConfig {
  groups: PlaylistGroup[];
}

interface PlaylistGroupsProps {
  user: any;
...
const PlaylistGroups: React.FC<PlaylistGroupsProps> = ({ 
  user, 
  setIsAuthenticated, 
  setUser, 
  currentTenant 
}) => {
  const [groupsConfig, setGroupsConfig] = useState<PlaylistGroupsConfig | null>(null);
  ...
  // Fetch groups and playlists when component mounts or tenant changes
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!tenant) {
          setGroupsConfig(null);
          setPlaylists([]);
          setLoading(false);
          return;
        }
        
        // Fetch both playlist groups and playlists in parallel
        const [groupsResponse, playlistsResponse] = await Promise.all([
        ...
        if (!groupsResponse.ok) {
          ...
          console.error('Server error response (groups):', errorText);
          throw new Error(`Server returned ${groupsResponse.status}: ${groupsResponse.statusText}`);
        }
        ...
        console.log('Server response data (groups):', groupsData);
        console.log('Server response data (playlists):', playlistsData);
        
        if (groupsData.success && playlistsData.success) {
          // Format to match our expected structure
          const groupsConfig: PlaylistGroupsConfig = {
            groups: groupsData.playlistGroups.map((group: any) => ({
              id: group.id,
              name: group.name,
              description: group.description,
              createdAt: group.createdAt,
              updatedAt: group.updatedAt,
              playTime: group.schedules?.map((schedule: any) => ({
          ...
          throw new Error(groupsData.message || playlistsData.message || 'Failed to fetch data');
        }
      } catch (err) {
  ...
  const handleCreateGroup = async () => {
    ...
    if (groupsConfig?.groups.some(g => g.name === newGroupName)) {
    ...
    try {
      ...
      if (data.success) {
        ...
        const updatedConfig = { 
          ...groupsConfig!, 
          groups: [...(groupsConfig?.groups || []), formattedGroup] 
        };
  ...
  const handleAddSchedule = async () => {
    ...
    // Get the currently selected group to check for overlap
    const currentGroup = groupsConfig?.groups.find(g => g.id === selectedGroup);
    if (!currentGroup) {
    ...
    const hasOverlap = currentGroup.playTime.some(schedule => {
    ...
    try {
      ...
      // Find the group to add the schedule to
      const group = groupsConfig!.groups.find(
      ...
      if (data.success) {
        ...
        const updatedConfig = JSON.parse(JSON.stringify(groupsConfig));
        const groupIndex = updatedConfig.groups.findIndex((g: any) => g.id === selectedGroup);
        updatedConfig.groups[groupIndex].playTime.push(newSchedule);
        updatedConfig.groups[groupIndex].updatedAt = new Date().toISOString();
    ...
    if (window.confirm(`Are you sure you want to delete this group?`)) {
      try {
        ...
        if (data.success) {
          ...
          const updatedGroups = groupsConfig.groups.filter(
            g => g.id !== groupId
          );
        ...
        setError(`Error deleting group: ${err instanceof Error ? err.message : String(err)}`);
        console.error('Error deleting group:', err);
      }
    }
  };

  const handleDeleteSchedule = async (groupId: string, scheduleIndex: number) => {
    if (!groupsConfig) return;
    ...
    if (window.confirm('Are you sure you want to delete this schedule entry?')) {
      try {
        ...
        // Find the group
        const groupIndex = groupsConfig.groups.findIndex(
          g => g.id === groupId
        );
        ...
        const groupResponse = await fetch(`/api/playlist-groups/${groupId}`, {
          method: 'GET',
          credentials: 'include',
          headers: {
            'Content-Type': 'application/json'
        ...
        if (!groupResponse.ok) {
          throw new Error(`Server returned ${groupResponse.status}: ${groupResponse.statusText}`);
        }
        ...
        if (!groupData.playlistGroup.schedules || groupData.playlistGroup.schedules.length <= scheduleIndex) {
        ...
        if (data.success) {
          ...
          const updatedPlayTime = [...groupsConfig.groups[groupIndex].playTime];
          updatedPlayTime.splice(scheduleIndex, 1);
          
          // Create updated config
          const updatedConfig = JSON.parse(JSON.stringify(groupsConfig));
          updatedConfig.groups[groupIndex].playTime = updatedPlayTime;
          updatedConfig.groups[groupIndex].updatedAt = new Date().toISOString();
  ...
  const handleExportJson = async () => {
    ...
    try {
      ...
      // Create a complete export with groups and all playlist content
      const exportData = {
      ...
      a.href = url;
      a.download = `playlist-groups-${currentTenant?.name || 'export'}.json`;
      ...
      reader.onload = async (e) => {
        try {
          ...
          if (!json.groups || !Array.isArray(json.groups)) {
            throw new Error('Invalid JSON structure. Must contain a groups array.');
          }
          ...
          // Now import groups by creating a new config
          const importGroups = window.confirm(
            `Do you want to import ${json.groups.length} playlist groups? 
            (This will only display the groups in the UI. Click 'Save' on each group to create it in the system)`
          );
          
          if (importGroups) {
            setGroupsConfig({ groups: json.groups });
          }
  ...
  return (
    <Layout user={user} handleLogout={handleLogout}>
      <div className="playlist-groups-container">
        <div className="playlist-groups-header">
          <h1>Playlist Group Management</h1>
          <div className="playlist-groups-actions">
            <input 
              type="file" 
              id="import-json" 
              accept=".json" 
        ...
        {!currentTenant && !localStorage.getItem('currentTenant') && (
          <div className="notification-bar">
            Please select a tenant from the sidebar dropdown to manage playlist groups.
          </div>
        )}
        
        {loading && <p>Loading playlist groups...</p>}
        {error && <p className="error-message">{error}</p>}
        
        {!loading && !error && (currentTenant || localStorage.getItem('currentTenant')) && !groupsConfig && (
          <div className="empty-state">
            <p>No playlist groups found for the selected tenant. Create or import a group to get started.</p>
        ...
        {groupsConfig && (
          <div className="group-content">
            ...
            {groupsConfig.groups.length === 0 ? (
              <div className="empty-message">No groups defined. Create a group to get started.</div>
            ) : (
                    ...
                    <div className="group-schedule">
                      {group.playTime.length === 0 ? (
                        ...
                        <table className="schedule-table">
                          ...
                          <tbody>
                            {group.playTime.map((schedule, index) => (
                              <tr key={`${group.id}-schedule-${index}`}>
                                ...
                                <td>
                                  <button 
                                    className="action-button delete"
                                    onClick={() => handleDeleteSchedule(group.id, index)}
          ...
          <div className="modal-overlay">
            <div className="modal-content">
              <div className="modal-header">
                <h2>Create New Playlist Group</h2>
                <button 
